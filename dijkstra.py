...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        for node in nodes:
            ...
            node.parent_b=None
            node.key_b=None
            node.processed=None #None:not visited; True:forward; False:backward
            
        Q_f=PriorityQueue()
        Q_b=PriorityQueue()
        source.key_f=NodeDistancePair(source,0)
        destination.key_b=NodeDistancePair(destination,0)
        Q_f.insert(source.key_f)
        Q_b.insert(destination.key_b)
        isforward=True
        imp_nodes=set()
        while True:
            Q=Q_f if isforward else Q_b
            key=Q.extract_min()
            u=key.node
            d_u=key.distance
            #something
            for v in u.adj:
                ...
                if isforward:
                    if v.key_f is None:
                        v.key_f=NodeDistancePair(v,temp)
                        v.parent_f=u
                        Q.insert(v.key_f)
                    elif v.key_f.distance>temp:
                        v.key_f.distance=temp
                        v.parent_f=u
                        Q.decrease_key(v.key_f)
                    if v.key_b is not None: imp_nodes.add(v)
                else:
                    if v.key_b is None:
                        v.key_b=NodeDistancePair(v,temp)
                        v.parent_b=u
                        Q.insert(v.key_b)
                    elif v.key_b.distance>temp:
                        v.key_b.distance=temp
                        v.parent_b=u
                        Q.decrease_key(v.key_b)
                    if v.key_f is not None: imp_nodes.add(v)
            if u.processed is None:
                u.processed=isforward
            elif u.processed != isforward:
                break
            isforward=not isforward

        w=imp_nodes.pop()
        dis=w.key_f.distance+w.key_b.distance
        while len(imp_nodes)!=0:
            x=imp_nodes.pop()
            temp=x.key_f.distance+x.key_b.distance
            if temp<dis:
                w=x
                dis=temp
